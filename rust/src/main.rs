...
fn register_post(user_form: Form<User>) -> Result<Redirect, DatastoreError> {
    let db = db::Db::new()?;
...
fn login_post(cookies: &CookieJar, login_form: Form<Login>) -> Result<Redirect, String> {
    let db = db::Db::new().map_err(|e| format!("Database error {:?}", e))?;
